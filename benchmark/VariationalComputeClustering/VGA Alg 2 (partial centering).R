...
#-------------------------------------------------------#
# Calculate variational lower bound (partial centering) #
#-------------------------------------------------------#

vlbc <- function(y,X,V,U,sigbeta,alpa,lama,alpb,lamb,alp,lam,sigdel,
                 mubetaq,sigbetaq,muetaq,sigetaq,alpaq,lamaq,mubq,sigbq,
...
lb <- ( 0.5*(k*(p+s2)+n*p)-0.5*nsum*log(2*pi)
+sum(-(0.5*s2+alpb)*log(lambq)+alpb*log(lamb)-lgamma(alpb)+lgamma(alpbq)
   +alpbq-lamb*alpbq/lambq)
...
for (i in 1:n) {
lb <- lb + 0.5*determinant(sigetaq[i,,])$modulus[1] }
...
for (i in 1:n) {
 for (j in 1:k) {
  ...
  t2 <- yi-Xi%*%muetaq[i,]-Vi%*%mubq[,j]
  t3 <- rep(alpq[,j]/lamq[,j],epm[i,])
lt[i,j] <- ( -0.5*alpaq[j]/lamaq[j]*(crossprod(muetaq[i,]-mubetaq[,j])
   + tr(sigetaq[i,,]+sigbetaq[j,,]))-0.5*(crossprod(t3*t2,t2)
+sum(sigetaq[i,,]*crossprod(t3*Xi,Xi))+sum(sigbq[j,,]*crossprod(t3*Vi,Vi)) ) )
}}         
lb <- lb + sum(qp*lt)

if (k >1) {
lb <- lb-0.5*determinant(as.matrix(sigdel))$modulus[1]-0.5*(k-1)*d*log(2*pi)-0.5*quadinv(mudelq,sigdel)
...
list(lb=lb) }
 ...
 #------------------------------------------------------------------#
 # Calculate variational lower bound simplified (partial centering) #
 #------------------------------------------------------------------#

vlbcshort <- function(y,X,V,U,sigbeta,alpa,lama,alpb,lamb,alp,lam,sigdel,
                 mubetaq,sigbetaq,muetaq,sigetaq,alpaq,lamaq,mubq,sigbq,
                 alpbq,lambq,alpq,lamq,mudelq,qp,vni,epm) {
n <- length(vni)
nsum <- sum(vni)
...
vlbcfix <- function(n,p,s2,g,nsum,sigbeta,alpa,lama,alpb,lamb,alp,lam,
                 mubetaq,sigbetaq,alpaq,lamaq,mubq,sigbq,
                 alpbq,lambq,alpq,lamq,qp,set) {
k <- length(alpa)
fixed <- seq(1,k,1)[-set]
alpbf <- alpb[fixed]
lambf <- lamb[fixed]
...
vlbcadd <- function(y,X,V,U,sigbeta,alpa,lama,alpb,lamb,alp,lam,sigdel,
                 mubetaq,sigbetaq,muetaq,sigetaq,alpaq,lamaq,mubq,sigbq,
...
#----------------------------------#
# VA Algorithm (partial centering) #
#----------------------------------#

MLMMc <- function(y,X,V,U,sigbeta,alpa,lama,alpb,lamb,alp,lam,sigdel,
                   vni,epm,qp,tol,stfit=NULL){
n <- length(vni)
nsum <- sum(vni)
p <- dim(X)[2]
d <- dim(U)[2]
...
t3 <- rep(alpq[,j]/lamq[,j],epm[i,])
t1 <- t1 + qp[i,j]*t3
t2 <- t2 + qp[i,j]*(alpaq[j]/lamaq[j]*mubetaq[,j]+crossprod(t3*Xi,yi-Vi%*%mubq[,j]))}
sigetaq[i,,] <- solve(sum(alpaq/lamaq*qp[i,])*diag(p) + crossprod(t1*Xi,Xi))
muetaq[i,] <- crossprod(sigetaq[i,,],t2)  }

# update sigbetaq and mubetaq #
for (j in 1:k) {
sigbetaq[j,,] <- solve(solve(sigbeta[j,,])+alpaq[j]/lamaq[j]*colSums(qp)[j]*diag(p))
mubetaq[,j] <- crossprod(sigbetaq[j,,],alpaq[j]/lamaq[j]*colSums(qp[,j]*muetaq)) }
...
t6 <- t6 + qp[i,j]*crossprod(t3*Vi,yi-Xi%*%muetaq[i,])} 
...
C[i,j] <- (-0.5*p*(log(lamaq[j])-digamma(alpaq[j]))
           -0.5*alpaq[j]/lamaq[j]*(crossprod(muetaq[i,]-mubetaq[,j])
                                    + tr(sigetaq[i,,]+sigbetaq[j,,]))
           -0.5*sum((log(lamq[,j])- digamma(alpq[,j]))*epm[i,])
           -0.5*(crossprod(t3*t2,t2)+ sum(sigetaq[i,,]*crossprod(t3*Xi,Xi))
...
t8 <- 0
for (i in 1:n) {
t8 <- t8 + qp[i,j]*(crossprod(muetaq[i,]-mubetaq[,j])+ tr(sigetaq[i,,]+sigbetaq[j,,])) }
...
Xi <- as.matrix(X[(sum(vni[1:i-1])+1):(sum(vni[1:i])),])
Vi <- as.matrix(V[(sum(vni[1:i-1])+1):(sum(vni[1:i])),])
t1 <- (matrix(rep(yi-Xi%*%muetaq[i,],k),ncol=k)-Vi%*%mubq)^2
t2 <- matrix(0,vni[i],k)
for (j in 1:k) {t2[,j] <- diag(Vi%*%sigbq[j,,]%*%t(Vi))}
t1 <- t1+matrix(rep(diag(Xi%*%sigetaq[i,,]%*%t(Xi)),k),ncol=k)+t2
ag <- aggregate(t1,list(rep(seq(1,g,1),epm[i,])),sum)
if (k==1) {
ag <- ag[,2]
if (any(epm[i,]==0)) {for (f in which(epm[i,]==0)) {ag <- insert(ag,f,0)}}
} else {
ag <- as.matrix(ag[,2:(k+1)])
if (any(epm[i,]==0)) {for (f in which(epm[i,]==0)) {ag <- insertRow(ag,f,rep(0,k))}}}
t0 <- t0 +0.5*matrix(rep(qp[i,],g),ncol=k,byrow=TRUE)*ag }
lamq <- lam+t0

lb <- vlbcshort(y,X,V,U,sigbeta,alpa,lama,alpb,lamb,alp,lam,sigdel,
                mubetaq,sigbetaq,muetaq,sigetaq,alpaq,lamaq,mubq,sigbq,
...
list (mubetaq=mubetaq,sigbetaq=sigbetaq,muetaq=muetaq,sigetaq=sigetaq,
mubq=mubq,sigbq=sigbq,alpaq=alpaq,lamaq=lamaq,alpbq=alpbq,lambq=lambq,
alpq=alpq,lamq=lamq,qp=qp,mudelq=mudelq,lb=lb,lbadj=lbadj,sigdelq=sigdelq)}
 

#-------------------------------------------------------#
# VA Algorithm (partial centering) for greedy algorithm #
#-------------------------------------------------------#

MLMMcp <- function(y,X,V,U,sigbeta,alpa,lama,alpb,lamb,alp,
...
qp[,c(cp,k)] <- pp[,c(cp,k)]*exp(C[,c(cp,k)])/matrix(rep(Q1+(Q1*Q2/Q3),times=length(c(cp,k))),ncol=length(c(cp,k)))
...
Xi <- as.matrix(X[(sum(vni[1:i-1])+1):(sum(vni[1:i])),])
Vi <- as.matrix(V[(sum(vni[1:i-1])+1):(sum(vni[1:i])),])
t1 <- (matrix(rep(yi-Xi%*%muetaq[i,],2),ncol=2)-Vi%*%mubq[,c(cp,k)])^2
t2 <- matrix(0,nrow=vni[i],ncol=2)
num <- 0
for (j in c(cp,k)) {
num <- num+1
t2[,num] <- diag(Vi%*%sigbq[j,,]%*%t(Vi))}
t1 <- t1+matrix(rep(diag(Xi%*%sigetaq[i,,]%*%t(Xi)),2),ncol=2)+t2
ag <- aggregate(t1,list(rep(seq(1,g,1),epm[i,])),sum)
ag <- cbind(ag[,2],ag[,3])
if (any(epm[i,]==0)) {for (f in which(epm[i,]==0)) {ag <- insertRow(ag,f,rep(0,2))}}
t0 <-t0 +0.5*matrix(rep(qp[i,c(cp,k)],g),ncol=2,byrow=TRUE)*ag }
lamq[,c(cp,k)] <- lam[,c(cp,k)]+t0

if (count==1) {lbinit <- vlbcfix(n,p,s2,g,nsum,sigbeta,alpa,lama,alpb,lamb,alp,lam,mubetaq,
                 sigbetaq,alpaq,lamaq,mubq,sigbq,alpbq,lambq,alpq,lamq,qp,c(cp,k))$lb}
lbaft <- vlbcadd(y,X,V,U,sigbeta,alpa,lama,alpb,lamb,alp,lam,sigdel,
                 mubetaq,sigbetaq,muetaq,sigetaq,alpaq,lamaq,mubq,sigbq,
...
list (mubetaq=mubetaq,sigbetaq=sigbetaq,muetaq=muetaq,sigetaq=sigetaq,mubq=mubq,
sigbq=sigbq,alpaq=alpaq,lamaq=lamaq,alpbq=alpbq,lambq=lambq,alpq=alpq,lamq=lamq,
qp=qp,mudelq=mudelq,lb=lb)
}


MLMMcpa <- function(y,X,V,U,sigbeta,alpa,lama,alpb,lamb,alp,
                 lam,sigdel,vni,epm,qp,alpq,lamq,alpaq,lamaq,
                 alpbq,lambq,mubetaq,sigbetaq,mubq,sigbq,set,tol){
n <- length(vni)
nsum <- sum(vni)
p <- dim(X)[2]
d <- dim(U)[2]
...
Xi <- as.matrix(X[(sum(vni[1:i-1])+1):(sum(vni[1:i])),])
Vi <- as.matrix(V[(sum(vni[1:i-1])+1):(sum(vni[1:i])),])
t1 <- (matrix(rep(yi-Xi%*%muetaq[i,],sel),ncol=sel)-Vi%*%mubq[,set])^2
t2 <- matrix(0,nrow=vni[i],ncol=sel)
num <- 0
for (j in set) {
num <- num+1
t2[,num] <- diag(Vi%*%sigbq[j,,]%*%t(Vi))}
t1 <- t1+matrix(rep(diag(Xi%*%sigetaq[i,,]%*%t(Xi)),sel),ncol=sel)+t2
ag <- aggregate(t1,list(rep(seq(1,g,1),epm[i,])),sum)
ag <- as.matrix(ag[,seq(2,by=1,length.out=sel)])
if (any(epm[i,]==0)) {for (f in which(epm[i,]==0)) {ag <- insertRow(ag,f,rep(0,sel))}}
...
list (mubetaq=mubetaq,sigbetaq=sigbetaq,muetaq=muetaq,sigetaq=sigetaq,mubq=mubq,
sigbq=sigbq,alpaq=alpaq,lamaq=lamaq,alpbq=alpbq,lambq=lambq,alpq=alpq,lamq=lamq,
qp=qp,mudelq=mudelq,lb=lb,lbadj=lbadj,sigdelq=sigdelq)}
...
greedyXW <- function(y,X,W,V,U,vni,epm,M,IGa,IGb,sigdelp,sigbetap) {
...
fitpre <- MLMMc(y,X,V,U,sigbeta,alpa,lama,alpb,lamb,alp,
...
nredz <- 0
for (m in 1:M) {
fitcan <- MLMMcp(y,X,V,U,sigbeta,alpa,lama,alpb,lamb,alp,
...
alpq <- matrix(fitpre$alpq[,-rp],nrow=g)
alpq <- cbind(alpq,matrix(fits[[cp]]$alpq[,c(cp,(K+1))],ncol=2))
lamq <- matrix(fitpre$lamq[,-rp],nrow=g)
...
fitprenew <- MLMMcpa(y,X,V,U,sigbeta,alpa,lama,alpb,lamb,alp,
                 lam,sigdel,vni,epm,qp,alpq,lamq,alpaq,lamaq,
...
lam <- matrix(IGb,g,k)
sigdel <- sigdelp*diag((k-1)*d)
fitfinal <- MLMMc(y,X,V,U,sigbeta,alpa,lama,alpb,lamb,alp,
...
MLMMpmerge <- function(y,X,W,V,U,IGa,IGb,sigbetap,sigdelp,vni,epm,tol,fit,m1,m2,type){
n <- length(vni)
nsum <- sum(vni)
p <- dim(X)[2]
d <- dim(U)[2]
